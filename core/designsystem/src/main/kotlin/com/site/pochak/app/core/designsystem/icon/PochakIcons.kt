package com.site.pochak.app.core.designsystem.icon

import com.site.pochak.app.core.designsystem.R

object PochakIcons {
    val Home = R.drawable.ic_home
    val HomeFilled = R.drawable.ic_home_filled
    val Post = R.drawable.ic_post
    val PostFilled = R.drawable.ic_post_filled
    val Camera = R.drawable.ic_camera
    val CameraFilled = R.drawable.ic_camera_filled
    val Alarm = R.drawable.ic_alarm
    val AlarmFilled = R.drawable.ic_alarm_filled
    val Profile = R.drawable.ic_profile
    val ProfileFilled = R.drawable.ic_profile_filled
    val CaptureButton = R.drawable.ic_capture_button
    val FlashOn = R.drawable.ic_flash_on
    val FlashOff = R.drawable.ic_flash_off
    val Search = R.drawable.ic_search
    val Delete = R.drawable.ic_delete
}